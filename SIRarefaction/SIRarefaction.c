...
#include "SIRarefaction.h"

static int  verbose  = FALSE;

static int  nLines   = 10;

static char *usage[] = {"SIRarefactionS - \n",
                        "Required parameters:\n",
...
{
  t_SIParams* ptSIParams = (t_SIParams *) pvParams;
  double dAlphaDD = ptSIParams->dAlpha*sqrt(x);
  double dBetaDD  = ptSIParams->dBeta*x;
  double dLogP0   = logLikelihood(0, dAlphaDD, dBetaDD, ptSIParams->dGamma);

  return (1.0 - exp(dLogP0)) - ptSIParams->dC;
}

double calcMu(double dC, t_SIParams *ptSIParams)
{
  double dMu = 0.0;

  ptSIParams->dC = dC;

  solveF(1.0, 1.0e10, fMu, ptSIParams, 1.0e-7, &dMu);
...
int main(int argc, char* argv[]){
  ...
  for(i = 0; i < nSamples; i++){
    adMu[i] = ((double) tData.nJ)*calcMu(tParams.dCoverage, &atSIParams[i]);
    //printf("%f\n", adMu[i]);
    //fflush(stdout);
  }
