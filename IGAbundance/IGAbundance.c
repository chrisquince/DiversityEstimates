...
#include "IGAbundance.h"

static int  verbose  = FALSE;

static int  nLines = 9;

static char *usage[] = {"IGAbundance - \n",
                        "Required parameters:\n",
			"   -a    abundance data\n",
...
void updateExpectations(double* adExpect, int nMax, double dAlpha, double dBeta, int nS, t_Data *ptData)
{
  int i = 0;
  ...
  for(i = 1; i <= nMax; i++){
    ...
    dLog = logLikelihood(nA, dAlpha, dBeta);
