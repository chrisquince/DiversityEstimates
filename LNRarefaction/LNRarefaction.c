...
#include "LNRarefaction.h"

static int  verbose  = FALSE;

static int  nLines   = 10;

static char *usage[] = {"LNRarefaction4 - \n",
                        "Required parameters:\n",
			"   -in   filename      parameter file\n",
			"   -c    float         desired coverage\n",
			"   -a    abundance data file\n",
                        "Optional:\n",
...
double fMu(double x, void* pvParams)
{
  ...
  double dMDD = ptLNParams->dMDash + x;
  double dLogP0 = logLikelihoodQuad(0, dMDD, ptLNParams->dV);

  return (1.0 - exp(dLogP0)) - ptLNParams->dC;
}

double calcMu(double dC, t_LNParams *ptLNParams)
{
  double dLogMu = 0.0;

  ptLNParams->dC = dC;

  solveF(0, 1.0e7, fMu, ptLNParams, 1.0e-7, &dLogMu);
...
int main(int argc, char* argv[]){
  ...
  for(i = 0; i < nSamples; i++){
    adMu[i] = ((double) tData.nJ)*calcMu(tParams.dCoverage, &atLNParams[i]);
    //printf("%f\n", adMu[i]);
    //fflush(stdout);
  }
